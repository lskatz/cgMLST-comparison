...
function maxMemory(){
  ...
  for i in `seq 1 $num_runs`; do
    #echo "$cmd => $out" >&3
    \time sh -c "$cmd" > /dev/null
  done 2>&1 | \
    grep -o -E '[0-9]+maxresident' | \
    sed 's/maxresident//' > $out
}

maxMemory "hostname" markdown/test.txt
maxMemory "colorid_bv build -b $campy_colorid_bxi -s 30000000 -n 2 -k 39 -t 1 -r $campy_colorid_fofn && colorid_bv search -ms -b $campy_colorid_bxi -q $campy_chewiedb/*.fasta" markdown/colorid.txt
maxMemory "EToKi.py MLSType -k sample -i $campy_asm -r $campy_etokirefs -d $campy_etokidb -o $campy_etoki_alleles" "markdown/etoki.txt"
maxMemory "singularity exec -B $TMPDIR:$TMPDIR -B $PWD:$PWD -B $chewie_input:/input -B $campy_chewiedb:/schema $chewie_cif chewBBACA.py AlleleCall -i /input --schema-directory /schema -o $TMPDIR/chewie.out --fr --cpu 1" "markdown/chewie.txt"
